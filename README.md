...
테스트 준비 중 AI 풀이의 반환 표현이 공개 예제와 맞지 않으면 보정 규칙을 선택하는 API 요청이 최대 1회 추가될 수 있습니다. 예를 들어 기대값 `32231`에 맞춰 계산 결과 `[3, 2, 2, 3, 1]`을 이어 붙여 반환한 뒤 전체 검증을 다시 진행합니다. 기대값과 사용자의 코드는 바꾸지 않으며, 계산 결과가 여전히 다르면 준비 실패로 표시합니다. 일반 실행 오류나 시간 초과에는 이 보정을 요청하지 않습니다.
