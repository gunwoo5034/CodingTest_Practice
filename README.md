...
히든 테스트를 포함한 제출을 한 번이라도 모두 통과하면 문제 목록과 풀이 화면에 **풀이 완료**가 표시됩니다. 공개 테스트의 실행 성공만으로는 완료 처리하지 않습니다. 완료 여부는 저장된 제출 기록을 기준으로 하므로 새로고침·재시작·이후 실패한 제출에도 유지되며, 문제 목록의 **풀이 완료** 필터로 모아 볼 수 있습니다.
