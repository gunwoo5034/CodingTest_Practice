...
export function LibraryPage() {
  ...
  return <div className="page library-page">
    ...
    <section className="library-toolbar"><label className="search-box"><Search size={18} /><input aria-label="문제 검색" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="문제 이름으로 검색" /></label><div className="filter-pills">{[['all','전체'],['ready','풀이 가능'],['draft','편집 중'],['needs_review','검토 필요']] .map(([id,label]) => <button key={id} className={filter === id ? 'active' : ''} onClick={() => setFilter(id)}>{label}</button>)}</div></section>
    ...
    {loading ? <div className="loading-grid">{[1,2,3].map((n) => <div className="skeleton card" key={n} />)}</div> : filtered.length === 0 ? <div className="empty"><BookOpen /><strong>{items.length ? '검색 결과가 없습니다' : '첫 문제를 등록해 보세요'}</strong><p>{items.length ? '다른 검색어나 상태를 선택해 보세요.' : '문제 텍스트나 이미지만 있으면 시작할 수 있습니다.'}</p>{!items.length && <Link className="button primary" to="/problems/new">문제 등록하기</Link>}</div> : <div className="problem-grid">{filtered.map((item, index) => <article className="problem-card" key={item.id}>
      <div className="card-top"><span className="problem-number">{String(index + 1).padStart(2, '0')}</span><StatusBadge status={item.status} /></div><h2>{item.title}</h2><p>테스트 리비전 {item.test_revision} · {formatDate(item.updated_at)}</p><div className="card-actions"><Link to={`/problems/${item.id}`} className="card-open">문제 풀기 <ArrowRight size={17} /></Link><Link aria-label={`${item.title} 편집`} className="icon-button" to={`/problems/${item.id}/edit`}><MoreHorizontal /></Link><button aria-label={`${item.title} 삭제`} className="icon-button danger" onClick={() => void remove(item)}><Trash2 size={17} /></button></div>
    </article>)}</div>}
  </div>;
}
