...
export function NewProblemPage() {
  ...
  const analyze = async () => { setBusy(true); setError(''); try { const created = await api.analyze({ text: text.trim() || null, image_base64: image?.base64 ?? null, image_mime: image?.mime ?? null }); navigate(`/problems/${created.id}/edit`, { state: { justAnalyzed: true } }); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } };
  const manual = async () => { if (!text.trim()) return setError('직접 등록하려면 문제 설명을 입력해 주세요.'); setBusy(true); try { const created = await api.createProblem({ title: text.trim().split('\n')[0].slice(0, 200), statement: text, example_explanation: '', constraints: [], signature: { parameters: [{ name: 'numbers', type: { base: 'int', dimensions: 1 } }], return_type: { base: 'int', dimensions: 0 } }, source_text: text }); navigate(`/problems/${created.id}/edit`); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } };
