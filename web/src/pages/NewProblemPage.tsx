...
export function NewProblemPage() {
  ...
  const [folders, setFolders] = useState<Folder[]>([]); const [folderId, setFolderId] = useState(''); const [folderLoading, setFolderLoading] = useState(true); const [folderError, setFolderError] = useState(''); const [params] = useSearchParams(); const requestedFolder = params.get('folder');
  const input = useRef<HTMLInputElement>(null); const folderLoadRevision = useRef(0); const navigate = useNavigate();
  ...
  const loadFolders = async () => { const revision = ++folderLoadRevision.current; setFolderLoading(true); setFolderError(''); try { const value = await api.folders(); if (folderLoadRevision.current !== revision) return; setFolders(value); setFolderId(requestedFolder && value.some((folder) => folder.id === requestedFolder) ? requestedFolder : ''); } catch (e) { if (folderLoadRevision.current === revision) setFolderError((e as Error).message); } finally { if (folderLoadRevision.current === revision) setFolderLoading(false); } };
  useEffect(() => { void loadFolders(); return () => { folderLoadRevision.current += 1; }; }, [requestedFolder]);
  const analyze = async () => { if (busy || folderLoading || folderError) return; setBusy(true); setError(''); try { const created = await api.analyze({ text: text.trim() || null, image_base64: image?.base64 ?? null, image_mime: image?.mime ?? null, folder_id: folderId || null }); navigate(`/problems/${created.id}/edit`, { state: { justAnalyzed: true } }); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } };
  const manual = async () => { if (busy || folderLoading || folderError) return; if (!text.trim()) return setError('직접 등록하려면 문제 설명을 입력해 주세요.'); setBusy(true); setError(''); try { const created = await api.createProblem({ title: text.trim().split('\n')[0].slice(0, 200), statement: text, example_explanation: '', constraints: [], signature: { parameters: [{ name: 'numbers', type: { base: 'int', dimensions: 1 } }], return_type: { base: 'int', dimensions: 0 } }, source_text: text, folder_id: folderId || null }); navigate(`/problems/${created.id}/edit`); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } };
  return <div className="page narrow-page"><header className="page-header compact"><div><span className="eyebrow">NEW PROBLEM</span><h1>문제를 가져오세요</h1><p>텍스트나 문제 이미지를 올리면 풀이에 필요한 구조를 정리합니다.</p></div></header>
    ...
    <label className="folder-select-field"><span>저장할 폴더</span><select aria-label="저장할 폴더" disabled={folderLoading || Boolean(folderError)} value={folderId} onChange={(event) => setFolderId(event.target.value)}><option value="">{folderLoading ? '폴더를 불러오는 중…' : '미분류'}</option>{folders.map((folder) => <option key={folder.id} value={folder.id}>{folder.name}</option>)}</select><small>나중에 문제 편집 화면에서도 바꿀 수 있습니다.</small></label>
    <div className="privacy-note"><Sparkles size={17} /><p><strong>분석은 이 버튼을 누를 때만 시작됩니다.</strong> 추출된 내용은 다음 화면에서 모두 수정할 수 있습니다.</p></div>
    {folderError && <div className="notice error">{folderError}<button onClick={() => void loadFolders()}>폴더 다시 불러오기</button></div>}{error && <div className="notice error">{error}</div>}
    <footer className="form-footer"><button className="button ghost large" disabled={busy || folderLoading || Boolean(folderError) || !text.trim()} onClick={() => void manual()}><FileText size={18} /> 직접 입력으로 등록</button><button className="button primary large" disabled={busy || folderLoading || Boolean(folderError) || (!text.trim() && !image)} onClick={() => void analyze()}><Sparkles size={18} /> {busy ? '분석 중…' : 'AI로 구조 분석'}</button></footer>
  </div>;
}
