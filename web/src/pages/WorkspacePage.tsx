...
function ResultsPanel({ job }: { job: Job | null }) {
  ...
  const completedSubmission = job.status === 'completed' && job.mode === 'submit' && job.summary?.all_passed && job.summary.total > 0;
  return <div className="results-panel">{job.summary && <div className={`result-summary ${job.summary.all_passed ? 'passed' : 'failed'}`}>{job.summary.all_passed ? <CheckCircle2 /> : <XCircle />}<div><strong>{job.summary.all_passed ? '모든 테스트를 통과했습니다' : `${job.summary.passed} / ${job.summary.total} 테스트 통과`}</strong><p>최대 {job.summary.max_time_ms.toFixed(1)} ms · {(job.summary.max_memory_kb / 1024).toFixed(1)} MB</p></div></div>}{completedSubmission && <Link className="button success result-completion-link" to="/">메인으로 돌아가기</Link>}{job.error && <div className="notice error">{job.error}</div>}<div className="result-list">{(job.results ?? []).map((result, i) => <details key={result.id} className={`result-row result-${result.status}`}><summary><span>{result.status === 'passed' ? <CheckCircle2 /> : <XCircle />}</span><strong>테스트 {i + 1}</strong><StatusBadge status={result.status} /><em>{result.time_ms.toFixed(1)} ms · {(result.memory_kb / 1024).toFixed(1)} MB</em></summary>{result.visibility !== 'hidden' ? <div className="result-detail"><div><span>기대값</span><code>{JSON.stringify(result.expected)}</code></div><div><span>실제값</span><code>{JSON.stringify(result.actual)}</code></div>{result.stdout && <div className="result-log"><span>출력</span><pre>{result.stdout}</pre></div>}{result.stderr && <div className="result-log stderr"><span>오류 출력</span><pre>{result.stderr}</pre></div>}</div> : <p className="hidden-note">히든 테스트의 입력과 출력은 공개되지 않습니다.</p>}</details>)}</div></div>;
}
