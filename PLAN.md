...
상태: 사용자 요청에 따라 구현 진행 중. 개발팀은 AGENTS.md의 역할 분담과 단일 구현 담당 규칙을 따른다.
...
| 언어 버전 | C++17·Python 3.12·Java 21 |
...
- C++·Python은 `solution`, Java는 `Solution.solution`을 호출한다. 매개변수와 반환 타입은 문제에 맞는 함수 틀로 제공한다.
...
아래 단계는 향후 사용자가 구현을 요청했을 때 실행한다. 팀 역할과 운영 규칙은 [AGENTS.md](AGENTS.md)를 따른다.

### 단계 1: 공통 계약과 프로젝트 기반

- [ ] 공통 데이터 계약, API 및 실행기 계약, 오류 구분을 먼저 확정한다.
- [ ] 프로젝트 구조, DB 저장, 문제 조회·저장, 작업 상태의 기반을 만든다.
- [ ] 문제와 DB가 재시작 후 유지되는지 확인한다.

### 단계 2: AI 없이 실행·채점하는 최소 기능

- [ ] 언어별 함수 틀과 실행 래퍼를 구현한다.
- [ ] 격리·자원 제한·오류 판정·반환값 비교를 구현한다.
- [ ] 직접 만든 예제 문제로 목록→풀이→실행→제출 흐름을 연결한다.
- [ ] 코드 저장, 사용자 테스트 편집, 제출 기록을 구현한다.

### 단계 3: 문제 분석과 테스트 생성

- [ ] 이미지·텍스트 입력, 분석 결과 확인·수정 화면을 구현한다.
- [ ] 기준 풀이·테스트 생성과 실제 실행 검증을 연결한다.
- [ ] 실패 상태·사용자 재생성·테스트 버전 보존을 구현한다.

### 단계 4: 요청형 풀이 도우미

- [ ] 질문·힌트·명시적인 정답 요청 흐름을 구현한다.
- [ ] 입력 중 자동 호출, 실행·제출 시 자동 호출이 없음을 확인한다.
- [ ] 대화 맥락, 토큰 사용량, API 실패 처리를 확인한다.

### 단계 5: 통합 검토와 로컬 실행 패키지

- [ ] Docker Compose, 로컬 환경변수 예제, macOS·Windows 실행 안내를 작성한다.
- [ ] 기능 통합, 컨테이너 종료·정리, 데이터 보존, API 키·히든 데이터 비노출을 검토한다.
- [ ] 실제 확인한 플랫폼과 확인하지 못한 플랫폼을 구분해 기록한다.
- [ ] 개인 데이터 없이 소스와 실행 설정만 공유 가능한지 확인한다.

### 필수 검증 시나리오

- C++·Python·Java에서 같은 문제의 정답·오답을 일관되게 판정한다.
