...
## AI solution output-format adaptation

Generation treats the existing signature and public expected values as its output contract. On public-example return-type errors or value mismatches only, it may request one additional structured AI decision selecting `none`, `concat_decimal`, `space_separated`, `comma_separated`, or `json_array`. AI chooses a generic formatting rule, not replacement solution code. Preserve both original algorithms in separate namespaces (including recursive global lookups); deterministic wrappers adapt only their returned values. Execute wrappers solely in the sandbox. Never pass expected values to the runner.

The Python runner identifies a return encoder TypeError with a private `failure: "return_type"` frame and nonzero exit; internal execution response adds `failure_kind: "return_type"` with runtime_error and no raw value. Keep resource-limit/OOM precedence. General solution exceptions never receive this marker. Public execution responses do not expose the new internal field. This allows preparation to distinguish return-shape problems without parsing stderr or disclosing unvalidated values.

Supported adaptation: one-dimensional native integer lists/tuples to scalar int/long/string; concatenation supports all three, other formats require string. Preserve already conforming scalar results. Reject bool/float/nested data and unsupported targets; retain runner type/range enforcement. Validate both adapted solutions against every original public example and all existing small/hidden preparation checks. No adaptation retry for input validation, compilation, resource-limit, infrastructure, or differential-check failures. Never change signature, public expected values, user code, or original examples to make verification pass.
